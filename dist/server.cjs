...
${originalPost.content}`,
  ...
  async function proxyToBackend(req, res, path2, method, body) {
    const authHeader = req.headers["authorization"] || "";
    const qs = Object.keys(req.query || {}).length ? "?" + new URLSearchParams(req.query).toString() : "";
    const targetUrl = `${MT5_BACKEND_URL}${path2}${qs}`;
    try {
      const fetchOptions = {
        method,
        headers: {
  ...
  app.post("/api/metatrader/reconnect", authenticate, async (req, res) => {
