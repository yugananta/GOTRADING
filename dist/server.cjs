...
var FALLBACK_POSTS = [
  {
    ...
    authorVerified: true,
    content: "Welcome to GoTrading Hub! \u{1F680} Explore live market pulse, advanced technical charts, institutional trading groups, and connect with fellow professional traders across Indonesia and Southeast Asia.",
    marketBias: "Bullish",
    likesCount: 142,
    commentsCount: 28,
    bookmarksCount: 45,
  ...
  },
  {
    ...
    authorCountry: "Indonesia",
    authorVerified: true,
    content: "XAUUSD (Gold) testing major resistance at 2385. Looking for a potential pullback to 2370 support before continuing the bullish trend. Manage your risk properly and keep an eye on US CPI data later today! \u{1F4CA}\u2728",
    marketBias: "Bullish",
    likesCount: 89,
    commentsCount: 14,
    bookmarksCount: 22,
  ...
  },
  {
    ...
    authorVerified: true,
    content: "EUR/USD breaking out of the 4-hour consolidation channel. Target set at 1.0920 with stop loss strictly below 1.0850. Strict risk management is key to long-term consistency in forex markets. \u{1F4C9}\u{1F4C8}",
    marketBias: "Bullish",
    likesCount: 64,
    commentsCount: 9,
    bookmarksCount: 15,
...
async function startServer() {
  ...
  const mockTechnicalDB = /* @__PURE__ */ new Map();
  app.post("/api/analysis/sync", async (req, res) => {
    const { symbol } = req.body;
    if (!symbol) return res.status(400).json({ error: "Symbol required" });
    const basePrice = symbol.includes("XAUUSD") ? 2350 : 1.08;
    const indicators = {
      ema20: basePrice * (1 + (Math.random() * 2e-3 - 1e-3)),
      ema50: basePrice * (1 + (Math.random() * 4e-3 - 2e-3)),
      ema200: basePrice * (1 + (Math.random() * 0.01 - 5e-3)),
      rsi: 30 + Math.random() * 40,
      macd: (Math.random() * 2 - 1).toFixed(4),
      adx: 15 + Math.random() * 30,
      vwap: basePrice,
      atr: (basePrice * 2e-3).toFixed(4),
    ...
    if (!mockTechnicalDB.has(symbol)) {
      mockTechnicalDB.set(symbol, {
        sentiment: {
          ...
          riskLevel: "Medium",
          signal: "WAIT",
          aiExplanation: "Initializing analysis...",
          updatedAt: (/* @__PURE__ */ new Date()).toISOString()
        },
        indicators: {
          ema20: 0,
...
${originalPost.content}`,
  ...
  const proxyToBackend = async (req, path2, method, customBody) => {
    ...
    const backendToken = import_jsonwebtoken2.default.sign(jwtPayload, tokenSecret, { expiresIn: "15m" });
    const url = `${backendUrl}${path2}`;
    const headers = {
