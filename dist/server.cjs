...
var AuthService = class {
  ...
  async login(email, password, ip, device) {
    ...
    return { accessToken: generateAccessToken({ id: user.id, email: user.email, role: user.role }), refreshToken };
  }
...
var authenticate = (req, res, next) => {
  ...
  try {
    const payload = import_jsonwebtoken2.default.verify(token, ACCESS_SECRET);
  ...
  async getCitiesByProvinceId(provinceId) {
    try {
      ...
      for (const city of rawCities) {
        ...
        const isJakartaVariant = cityName === "Jakarta Barat" || cityName === "Jakarta Pusat" || cityName === "Jakarta Selatan" || cityName === "Jakarta Timur" || cityName === "Jakarta Utara" || cityName === "DKI Jakarta";
        if (isJakartaVariant) {
          if (!addedJakarta) {
...
async function startServer() {
  ...
  app.all(["/api/auth", "/api/auth/*"], async (req, res) => {
    ...
    const targetUrl = `${BACKEND_API_URL2}${req.originalUrl}`;
    ...
    try {
      ...
      clearTimeout(timeout);
      console.error(`[AUTH PROXY ERROR] ${req.method} ${req.originalUrl} -> ${targetUrl}:`, err.message);
  ...
  function generateDynamicDiagnosis(analysisType, customPrompt, tradeMetrics) {
    const {
      accountInfo = {},
      stats = {},
      symbolBreakdown = [],
    ...
    const worstSymbol = [...symbolBreakdown].sort((a, b) => a.totalPL - b.totalPL)[0] || { symbol: "XAUUSD", totalPL: 0, count: 0, winRate: 0 };
    const bestSymbol = [...symbolBreakdown].sort((a, b) => b.totalPL - a.totalPL)[0] || { symbol: "XAUUSD", totalPL: 0, count: 0, winRate: 0 };
    const bestSession = [...sessionBreakdown].sort((a, b) => b.winRate - a.winRate)[0] || { session: "London", winRate: 65, totalPL: 0, count: 0 };
    const worstSession = [...sessionBreakdown].sort((a, b) => a.winRate - b.winRate)[0] || { session: "Asian", winRate: 40, totalPL: 0, count: 0 };
    ...
    switch (analysisType) {
      case "leaks": {
        const typeLeak = buyStats.winRate < sellStats.winRate ? `Eksekusi posisi BUY mengalami kendala dengan win rate ${(buyStats.winRate || 0).toFixed(1)}% (P&L: ${buyStats.totalPL >= 0 ? "+" : ""}$${buyStats.totalPL.toFixed(2)}), lebih rendah dibanding SELL (${(sellStats.winRate || 0).toFixed(1)}%).` : `Eksekusi posisi SELL mengalami kendala dengan win rate ${(sellStats.winRate || 0).toFixed(1)}% (P&L: ${sellStats.totalPL >= 0 ? "+" : ""}$${sellStats.totalPL.toFixed(2)}), lebih rendah dibanding BUY (${(buyStats.winRate || 0).toFixed(1)}%).`;
        return {
          title: "Audit Kebocoran Modal & Risk Leakage",
          content: totalTrades > 0 ? `AI Engine mengaudit ${totalTrades} transaksi riil di akun MT5 Anda. Kebocoran performa utama terjadi pada pasangan ${worstSymbol.symbol} dengan akumulasi P&L ${worstSymbol.totalPL >= 0 ? "+" : "-"}$${Math.abs(worstSymbol.totalPL).toFixed(2)} dari ${worstSymbol.count} tiket order. ${typeLeak} Kerugian transaksi tunggal terbesar yang tercatat adalah -$${Math.abs(worstTrade).toFixed(2)}.` : `AI Engine telah memindai jurnal transaksi MT5 Anda. Saat ini belum tercatat kebocoran risiko signifikan karena transaksi tertutup masih minim.`,
      ...
      case "evidence": {
        return {
          ...
          content: totalTrades > 0 ? `Audit terverifikasi atas ${totalTrades} tiket order MetaTrader 5 riil. Total Profit: ${stats.winTrades || 0} tiket (Rata-rata: +$${avgWin.toFixed(2)}), Total Loss: ${stats.lossTrades || 0} tiket (Rata-rata: -$${avgLoss.toFixed(2)}). Rasio Realized Reward-to-Risk terealisasi sebesar ${rewardToRisk.toFixed(2)}:1 dengan Profit Factor ${profitFactor.toFixed(2)}.` : `Memeriksa tiket order terminal MetaTrader. Belum ditemukan riwayat tiket tertutup untuk dianalisis secara mendalam.`,
          ...
          confidence: 95,
          riskBias: rewardToRisk < 1 ? "Elevated" : "Low",
          keyInsights: [
      ...
      case "steps": {
        const coachingNote = winRate >= 50 ? `Win rate akun Anda solid di ${winRate.toFixed(1)}%. Fokus utama sekarang adalah memaksimalkan profit factor dengan menerapkan trailing stop pada pemenang.` : `Win rate akun Anda saat ini di angka ${winRate.toFixed(1)}%. Prioritas utama adalah memperketat filter konfirmasi teknikal dan menghindari setup probabilitas rendah.`;
        return {
      ...
      case "sessions": {
        return {
          ...
          content: totalTrades > 0 ? `AI Engine menganalisis korelasi waktu pembukaan & penutupan transaksi pada sesi London, New York, dan Asia. Sesi performa tertinggi Anda adalah ${bestSession.session} dengan Win Rate ${bestSession.winRate.toFixed(1)}% dan Net P&L ${bestSession.totalPL >= 0 ? "+" : ""}$${bestSession.totalPL.toFixed(2)} dari ${bestSession.count} tiket order.${worstSession.session !== bestSession.session ? ` Sebaliknya, sesi ${worstSession.session} mencatat win rate terendah (${worstSession.winRate.toFixed(1)}%).` : ""}` : `Menganalisis parameter sesi trading. Data riil menunjukkan sesi overlap London dan New York memiliki likuiditas tertinggi untuk Gold (XAUUSD) dan Forex major.`,
  ...
  app.post("/api/ai/journal-diagnosis", async (req, res) => {
    try {
...
${symbolBreakdown.length > 0 ? symbolBreakdown.map((s) => `  - ${s.symbol}: ${s.count} trades, WinRate ${(s.winRate || 0).toFixed(1)}%, Net P&L ${s.totalPL >= 0 ? "+" : ""}$${(s.totalPL || 0).toFixed(2)}`).join("\n") : "  - None"}
\u2022 Direction Breakdown:
...
${sessionBreakdown.length > 0 ? sessionBreakdown.map((sess) => `  - ${sess.session}: ${sess.count} trades, WinRate ${(sess.winRate || 0).toFixed(1)}%, Net P&L ${sess.totalPL >= 0 ? "+" : ""}$${(sess.totalPL || 0).toFixed(2)}`).join("\n") : "  - General session data"}
\u2022 Recent 15 Execution Tickets:
${recentTickets.length > 0 ? recentTickets.slice(0, 15).map((t) => `  #${t.ticket} | ${t.symbol} | ${t.type} ${t.volume || t.lots} Lots | Open: ${t.openPrice} -> Close: ${t.closePrice} | P&L: ${t.pl >= 0 ? "+" : ""}$${t.pl} | Closed: ${t.closeTime}`).join("\n") : "  - None"}
--------------------------------------
...
`;
  ...
  app.get("/api/groups/stats", async (req, res) => {
    try {
      ...
      const now = Date.now();
      if (groupStatsCache[cacheKey] && now - groupStatsCache[cacheKey].timestamp < 1e4) {
      ...
      if (supabase) {
        ...
        cityUserCount = users.filter((u) => u.city && u.city.toLowerCase() === city.toLowerCase()).length;
        provinceUserCount = users.filter((u) => u.province && u.province.toLowerCase() === province.toLowerCase()).length;
        cityChatCount = messages.filter((m) => m.receiverId === cityGroupId).length;
        provinceChatCount = messages.filter((m) => m.receiverId === provinceGroupId).length;
        cityPostCount = posts.filter((p) => p.groupId === cityGroupId).length;
        provincePostCount = posts.filter((p) => p.groupId === provinceGroupId).length;
      }
      const resultData = {
        city: {
  ...
  const resolveReferrer = async (rawRef) => {
  ...
  app.get("/api/auth/check-referral", async (req, res) => {
    try {
      ...
      try {
        fetch(`${BACKEND_API_URL}/api/auth/register`, {
          ...
          body: JSON.stringify({
            ...
            full_name: `${firstName} ${lastName}`.trim(),
            whatsapp: whatsappNumber || "",
            referralCode: referrerUser?.referral_code || incomingRef || void 0
          })
        }).then(async (r) => {
          if (r.ok) {
  ...
  app.get("/api/users", async (req, res) => {
    ...
    let list = await userRepo.list({
      ...
      province: province ? String(province) : void 0,
      country: country ? String(country) : void 0,
    ...
    let parsedLimit = limit ? parseInt(limit, 10) : 0;
    if (parsedLimit > 0) {
...
${originalPost.content}`,
  ...
  app.delete("/api/notifications/:notificationId", async (req, res) => {
    ...
    if (req.db && req.db.notifications) {
      req.db.notifications = req.db.notifications.filter((n) => n.id !== req.params.notificationId);
    }
    res.json({ success: true });
  });
  ...
  app.all(["/api/metatrader", "/api/metatrader/*", "/api/ib", "/api/ib/*", "/api/settings", "/api/settings/*", "/api/validations", "/api/validations/*"], async (req, res) => {
