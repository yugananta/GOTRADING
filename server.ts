...
`;
    ...
    try {
      ...
      return {
        ...
        payouts: payouts.map((p: any) => ({
          id: p.id,
          date: p.processed_at || p.requested_at || p.created_at || new Date().toISOString(),
      ...
      console.error('[IB DB] Error querying IB summary:', e);
      return {
        referralCode: `GOTRADING-${userId.slice(0, 6).toUpperCase()}`,
        referralLink: `https://gotrading.id/register?ref=GOTRADING-${userId.slice(0, 6).toUpperCase()}`,
        totalCommission: 0,
        totalPayout: 0,
        pendingPayout: 0,
        totalReferrals: 0,
  ...
  app.all(["/api/metatrader", "/api/metatrader/*", "/api/ib", "/api/ib/*"], async (req: any, res) => {
    ...
    try {
      ...
      // If BE responds ok, return BE response directly
      if (backendRes.ok) {
      ...
      // If BE returns 401/404/500 for /api/ib routes, try direct Supabase query if user token is valid
      if (req.originalUrl.startsWith('/api/ib')) {
        let userId = '';
        ...
        if (userId) {
          if (req.originalUrl.includes('/api/ib/me/commissions')) {
            const commissions = await getIbCommissionsFromDb(userId);
            return res.json({ success: true, data: commissions });
          }
          if (req.originalUrl.includes('/api/ib/me')) {
            const summary = await getIbSummaryFromDb(userId);
      ...
      // Fallback for /api/ib routes when backend is unreachable
      if (req.originalUrl.startsWith('/api/ib')) {
        let userId = '';
        const authHeader = req.headers.authorization;
        const token = authHeader && authHeader.split(' ')[1];
