...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Handle stats updates if broker is connected or simulated live feeds
  useEffect(() => {
    if (connectedBroker) {
      // simulate nice trading fluctuations using setInterval (non-fetch, local logic)
      const interval = setInterval(() => {
        setTradingStats(prev => {
          const rawPL = parseFloat(prev.todayPL.replace('+$', '').replace('-$', ''));
          const delta = (Math.random() - 0.45) * 12; // positive bias
  ...
  const fetchMetaTraderData = async () => {
    try {
      ...
      console.error("Error fetching MetaTrader data:", e);
    }
  };
  ...
  const syncMetaTrader = async () => {
    try {
      ...
      console.error("Error syncing MetaTrader:", err);
    }
  };
