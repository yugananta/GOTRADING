          ...
          <div className="p-4 sm:p-6 overflow-y-auto space-y-5 no-scrollbar flex-1 flex flex-col items-center">
            ...
            <div className="w-full flex justify-center py-1">
              ...
              >
                ...
                {/* Main Hero Net P&L */}
                <div className="relative z-10 py-3 my-1 text-center bg-current/5 rounded-2xl border border-current/10 p-4 space-y-1">
                  <span className={`text-[9px] font-black uppercase tracking-widest block ${themeStyle.subText}`}>
                    Daily Net Result
                  </span>
                  
                  <div className="flex items-center justify-center gap-2">
                    <h2 className={`text-3xl sm:text-4xl font-black tracking-tight font-roboto ${themeStyle.accentText}`}>
                      {isProfit ? '+' : ''}${Math.abs(data.netPL).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </h2>
                  </div>

                  {showPercent && (
                    <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black bg-current/10 font-roboto">
                      <TrendingUp size={11} className={isProfit ? 'text-emerald-500' : 'text-rose-500'} />
                      <span>{isProfit ? '+' : ''}{data.netPLPercent.toFixed(2)}% Return</span>
                    </div>
                  )}
                </div>
                ...
                <div className="relative z-10 grid grid-cols-2 gap-2.5 pt-3">
                  <div className={`p-3 rounded-2xl border ${themeStyle.boxBg}`}>
                    <span className={`text-[8.5px] font-bold uppercase tracking-wider block mb-0.5 ${themeStyle.subText}`}>
                      Win Rate
                    </span>
                    <div className="flex items-baseline justify-between">
                      <span className={`text-sm font-black font-roboto ${themeStyle.statVal}`}>
                        {data.winRate.toFixed(0)}%
                      </span>
                      <span className={`text-[9px] font-bold font-roboto ${themeStyle.subText}`}>
                  ...
                  <div className={`p-3 rounded-2xl border ${themeStyle.boxBg}`}>
                    ...
                    <span className="text-sm font-black font-roboto text-emerald-500">
                      +${data.bestTrade.toFixed(2)}
                    </span>
                  </div>
